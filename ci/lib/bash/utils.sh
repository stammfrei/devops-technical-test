...
log() {
	...
	case $level in
	i | I | info | INFO) level="${_color_green}INFO${_color_reset}" ;;
	w | W | warn | WARN) level="${_color_yellow}WARN${_color_reset}" ;;
	e | E | error | ERROR) level="${_color_red}ERROR${_color_reset}" ;;
	d | D | debug | DEBUG) level="${_color_blue}DEBUG${_color_reset}" ;;
	*)
